...
Defines the requirements for the `PATCH /api/v1/users/me/goals` endpoint, which allows the authenticated user to update their daily learning goals (`dailyGoalMinutes` and/or `dailyGoalLessons`) persisted in `UserPreferences`.
