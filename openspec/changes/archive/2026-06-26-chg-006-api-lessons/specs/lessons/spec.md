...
O sistema SHALL registrar a conclusão de uma lição em uma transação atômica que: (1) cria `LessonCompletion`, (2) incrementa `UserProgress.totalXP`, (3) atualiza `currentStreakDays` e `lastActivityDate`. A transação usa `prisma.$transaction` com isolamento Serializable. Em caso de falha parcial, toda a operação é revertida. RBAC mínimo: `USER`.
...
- **THEN** sistema cria `LessonCompletion`, incrementa `totalXP`, atualiza streak e retorna resultado
